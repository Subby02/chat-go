...
//// 신고 목록 API
router.get('/api/object/lost/list/:num', async (req, res) => {
    try {
...
//해당하는글 들어가기
router.get('/api/object/lost/detail/:id', async (req, res) => {
    try {
...
//글 작성하기
router.post('/api/object/lost/write', async (req, res) => {
    try {
        const {
            user_id,
            date,
...
//해당하는 글 수정하기
router.get('/api/object/lost/edit/:id', async (req, res) => {
    try {
        const post = await Object_lost.findById(req.params.id);
...
router.put('/api/object/lost/edit/:id', async (req, res) => {
    try {
        const { uniq } = req.body;
