...
const getInitialStateFromQuery = (location) => {
  ...
  return {
    ...
    filters: {
      ...
      fdYmdStart: params.get("fdYmdStart") || "",
      fdYmdEnd: params.get("fdYmdEnd") || "",
      si: params.get("si") || "",
      sgg: params.get("sgg") || "",
      emd: params.get("emd") || "",
    },
...
const ReportBoard = () => {
    ...
    async (pageToFetch) => {
      ...
      try {
        const response = await axios.get(
          `http://localhost:5000/api/object/get/search?${queryString}`
        );
  ...
  useEffect(() => {
    ...
    if (activeFilters.fdYmdStart)
      params.set("fdYmdStart", activeFilters.fdYmdStart);
    if (activeFilters.fdYmdEnd) params.set("fdYmdEnd", activeFilters.fdYmdEnd);
    ...
    if (location.search !== newSearch) {
      nav(`/object/get${newSearch}`, { replace: true });
    }
